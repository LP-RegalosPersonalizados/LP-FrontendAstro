...
export default function CartItemRow({ item }: Props) {
  ...
  // Estado local para manejar la edición de cantidad por teclado
  const [localQuantity, setLocalQuantity] = useState<string>(String(item.quantity));

  // Mantener el estado local sincronizado con la store
  useEffect(() => {
  ...
  const handleBlur = () => {
    // Si el input quedó vacío al salir, establecer mínimo 1
    if (localQuantity === '') {
      setLocalQuantity('1');
