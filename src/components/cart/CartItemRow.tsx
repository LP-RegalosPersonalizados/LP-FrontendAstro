...
export default function CartItemRow({ item }: Props) {
  const { removeItem, updateQuantity, updatePersonalization, mode } = useCart();

  return (
    <li className="bg-neutral-light/30 rounded-xl p-3 space-y-2">
      <div className="flex items-start justify-between gap-2">
        <p className="font-medium text-sm text-gray-800 leading-snug flex-1">{item.name}</p>
        <button
      ...
      <div className="flex items-center gap-2">
        ...
        {mode === 'business' ? (
          <input
            ...
            className="w-20 text-sm border border-neutral-light rounded-lg px-2 py-1 text-center focus:outline-none focus:ring-2 focus:ring-primary"
