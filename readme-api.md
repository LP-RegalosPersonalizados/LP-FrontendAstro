...
- Los GET públicos de categorías y productos llevan header `Cache-Control` para que el edge cachee la respuesta. Ver [Caché](#caché).
...
El backend usa una única hoja `productos` en Google Sheets (vía SheetDB). Las categorías **no tienen hoja propia**: se derivan de los productos. Ver [Categorías: modelo y ciclo de vida](#categorías-modelo-y-ciclo-de-vida).
...
`200` con el objeto trabajo. `404` con `{ "error": "Trabajo no encontrado" }`.
...
`201` con el objeto trabajo. `400` con `{ "error": "title es requerido" }`.

### `PUT /api/trabajos/:id` — Actualizar trabajo

Acepta los mismos campos que la creación. `200` con el trabajo actualizado; `404` si no existe.

### `DELETE /api/trabajos/:id` — Eliminar trabajo

`200` con `{ "message": "Trabajo eliminado", "id": "..." }`.
...
Misma forma que productos: `{ "creates": [...], "updates": [...] }`. Respuesta con `created`, `updated` y `failed`.

### `POST /api/trabajos/batch/delete` — Eliminar múltiples trabajos

Misma forma: `{ "ids": [...] }`. Respuesta con `deleted` y `failed`.
...
- **Lecturas con filtro por ID** (búsqueda `search`): no se cachean, siempre consultan a SheetDB.
- Si SheetDB falla y hay datos en caché, se sirve la caché como fallback.

Capas adicionales:

- Los GET públicos de categorías y productos por categoría devuelven `Cache-Control: public, s-maxage=3600, stale-while-revalidate=259200`, permitiendo que Vercel cachee la respuesta en el edge por 1 hora con revalidación hasta 72 horas.
